...
class ThermalPrinter {
  ...
  printTableRow(rowData) {
    const printerWidth = this.config.width;
    let start = 0;
    rowData.forEach(element => {
      const colLength = Math.floor(printerWidth * element.width * (element.size || 1))
      // const text = element.text
      // print( this.padCol(element.text, colLength))
      const str = element.text.toString();
      let padSpaces = colLength - (str.length * (element.size || 1));
      let spaces = ""
      if (padSpaces > 0) {
        spaces = Array(padSpaces + 1).join(' ')
      }
      if (spaces.length > 0 && element.align == 'left') {
        this.print(spaces)
        this.print(element.text)
      }
